...
export class EnvironmentVariableInfoStale implements IEnvironmentVariableInfo {
	...
	private _getInfo(scope: EnvironmentVariableScope | undefined): string {
		...
		message += getMessage(this._collection, scope, this._extensionService, extSet);
		return message;
	}
...
function getMessage(collection: IMergedEnvironmentVariableCollection, scope: EnvironmentVariableScope | undefined, extensionService: IExtensionService, extSet: Set<string>): string {
	let message = '\n';
